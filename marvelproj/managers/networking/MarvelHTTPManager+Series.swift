...
extension MarvelHTTPManager{
    ...
    func fetchSeries(characterID: Int, completion: @escaping (_ series: [Series], _ error: Error?) -> Void){
        ...
        let url = queryBuilder.query(ts: ts, apikey: dict.publicKey, hash: hash, characterID: characterID, detailExtension: "series", limit: LIMIT_OF_FETCH_COLLECTION_VIEW_SERIES)
